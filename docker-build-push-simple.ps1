# Simple Docker build and push script for migrant-fe-portal
...
docker build --build-arg NEXT_PUBLIC_API_URL=$API_URL -t $FULL_IMAGE_NAME .
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Run the following command to add the registry to insecure registries:" -ForegroundColor Yellow
    Write-Host "docker-build-push.ps1" -ForegroundColor Cyan
    exit 1
}
