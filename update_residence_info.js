...
// Path to the file
const filePath = path.join(__dirname, 'src', 'app', '(public)', 'application', 'residence-info', 'page.tsx');

// Read the file
let content = fs.readFileSync(filePath, 'utf8');

// Find the onSubmit function
const onSubmitPosition = content.indexOf('const onSubmit = async (formValues: ResidenceInfoFormValues)');
...
if (dataObject.includes('currentStep:')) {
  ...
  const newDataObject = dataObject.replace(
    '      };',
    '        currentStep: 40, // Update to parents-info step\n      };'
  );
  ...
  console.log('Successfully updated the residence-info page with currentStep: 40');
}
