...
export function PhoneInput({ className, value, onChange, ...props }: PhoneInputProps) {
  ...
  const extractCountryCode = (val: string) => {
    for (const country of countries) {
      if (val.startsWith(country.dial_code)) {
        return {
          dial_code: country.dial_code,
          phoneNumber: val.substring(country.dial_code.length)
        };
      }
    }
    return { dial_code: "+255", phoneNumber: val }; // Default to Tanzania if no match
  };

  const { dial_code, phoneNumber } = extractCountryCode(value);
  const [open, setOpen] = React.useState(false);
  const [selectedCountry, setSelectedCountry] = React.useState(
    countries.find((country) => country.dial_code === dial_code) || countries[0]
  );
  ...
  return (
    <div className={cn("flex flex-col", className)}>
      <div className="relative w-full border border-input rounded-md bg-background overflow-hidden">
        <div className="flex">
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                role="combobox"
                aria-expanded={open}
                className="flex items-center h-10 px-3 py-2 border-r border-input rounded-none bg-muted/20 hover:bg-muted/30"
              >
                  ...
                  <CommandGroup>
                    {countries.map((country) => (
                      ...
                      >
                        ...
                        <span className="font-medium mr-2">{country.dial_code}</span>
